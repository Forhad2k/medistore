...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [items, setItems] = useState<CartItem[]>(() => {
    try {
      ...
      return parsed.filter((item: any) => item && item.medicine && typeof item.medicine.price === 'number' && item.quantity > 0);
    } catch { return []; }
  });
  ...
  const totalAmount = items.reduce((acc, i) => {
    if (!i.medicine || !i.medicine.price) return acc;
    return acc + Number(i.medicine.price) * i.quantity;
  }, 0);
